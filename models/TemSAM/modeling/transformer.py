...
class TwoWayTransformer2(nn.Module):
    def __init__(
        ...
        self.refframe_attn_to_image=Attention3(embedding_dim, num_heads)
    ...
    def forward(
        ...
        bs, c, h, w = image_embedding.shape
        image_embedding = image_embedding.flatten(2).permute(0, 2, 1)
        refframe_embedding = refframe_embedding.flatten(2).permute(0, 2, 1)
        image_pe = image_pe.flatten(2).permute(0, 2, 1)
        # Prepare queries
        queries = point_embedding
        keys = image_embedding
        reff_keys=refframe_embedding+image_pe

        attn_out=self.refframe_attn_to_image(q=keys+image_pe, k=reff_keys, v=reff_keys, mask_feat=mask_feat)
        # attn_out=self.refframe_attn_to_image(q=keys+image_pe, k=reff_keys, v=reff_keys, mask_feat=mask_feat[:,1:2,:])
        keys+=attn_out
...
class TwoWayAttentionBlock2(nn.Module):
    ...
    def forward(
        self, queries: Tensor, keys: Tensor, query_pe: Tensor, key_pe: Tensor,refframe_embedding:Tensor, mask_feat: Tensor
    ) -> Tuple[Tensor, Tensor]:
...
class Attention2(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor, mask_feat: Tensor) -> Tensor:
        ...
        mask_feat = torch.repeat_interleave(mask_feat, self.num_heads, dim=1)#1,8,5,2500
        
        attn = attn / math.sqrt(c_per_head)#1,8,2500,6??
